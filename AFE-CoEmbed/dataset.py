...
class Example(AbstractExample):
    def __init__(self, instance):
        ...
        # for gnn
        # for gnn input
        self.node_value = instance['node_value']
        self.node_type = instance['node_type']
        self.node_action = instance['node_action']
        self.node_as_output = instance['node_as_output']

        self.edge_prt2ch = instance['edge_prt2ch']
        self.edge_prev2next = instance['edge_prev2next']
        self.edge_align = instance['edge_align']
        self.edge_com2sub = instance['edge_com2sub']
    ...
    @property
    def node_value_as_output_tokens(self):
        return [x for i, x in enumerate(self.node_value) if self.node_as_output[i]]
    ...
    def get_both_ext_vocab(self, base_vocab: VocabEntry = None):
        if not self.both_ext_vocab:
            ...
            self.both_ext_vocab = ExtVocabEntry(base_vocab, self.src_tokens + self.new_code_tokens +
                                                self.node_value_as_output_tokens)
    ...
    @classmethod
    def pretty_dump_examples_with_results_to_csv(cls, examples: List, filename: str, our_name: str,
                                                 our_results: List[str],
...
class Batch(object):
    ...
    @property
    def node_tokens(self) -> List[List[str]]:
        return [e.node_value for e in self.examples]

    @property
    def node_as_output(self) -> List[List[bool]]:
        return [e.node_as_output for e in self.examples]

    @property
    def edge_prt2ch(self) -> List[List[List]]:
        return [e.edge_prt2ch for e in self.examples]

    @property
    def edge_prev2next(self) -> List[List[List]]:
        return [e.edge_prev2next for e in self.examples]

    @property
    def edge_align(self) -> List[List[List]]:
        return [e.edge_align for e in self.examples]

    @property
    def edge_com2sub(self) -> List[List[List]]:
        return [e.edge_com2sub for e in self.examples]

    @property
    def node_type_tokens(self) -> List[List[str]]:
        return [e.node_type for e in self.examples]

    @property
    def node_action_tokens(self) -> List[List[str]]:
        return [e.node_action for e in self.examples]
    ...
    def get_node_tensors(self, node_vocab: VocabEntry, action_vocab: VocabEntry, type_vocab: VocabEntry,
                         device: torch.device):
        value_tensor = node_vocab.to_input_tensor(self.node_tokens, device)
        type_tensor = type_vocab.to_input_tensor(self.node_type_tokens, device)
        action_tensor = action_vocab.to_input_tensor(self.node_action_tokens, device)

        return value_tensor, type_tensor, action_tensor

    def get_gnn_other_inputs(self):
        return (self.node_as_output, self.edge_prt2ch, self.edge_prev2next, self.edge_align, self.edge_com2sub)
    ...
    def get_node_lens(self):
        return [len(sent) for sent in self.node_tokens]
    ...
    def get_both_ext_tensor(self, dec_nl_vocab: VocabEntry, device: torch.device) -> Tuple[Tensor, Tensor]:
        ...
        for e in self:
            ...
            node_word_ids.append(ext_vocab.words2indices(e.node_value_as_output_tokens))
        ...
        node_tensor = ids_to_input_tensor(node_word_ids, base_vocab[PADDING], device)
        return src_tensor, code_tensor, node_tensor
    ...
    def construct_encoder_input(self, code_vocab, action_vocab, node_action_vocab, nl_vocab, node_vocab, type_vocab,
                                device):
        code_tensor_a, code_tensor_b, action_tensor = self.get_code_change_tensors(code_vocab, action_vocab, device)
        node_value_tensor, node_type_tensor, node_action_tensor = self.get_node_tensors(node_vocab, node_action_vocab,
                                                                                        type_vocab, device)
        src_tensor = self.get_src_tensor(nl_vocab, device)

        code_lens = self.get_code_lens()
        src_lens = self.get_src_lens()
        node_lens = self.get_node_lens()

        gnn_other_inputs = self.get_gnn_other_inputs()
        return code_tensor_a, code_tensor_b, action_tensor, code_lens, src_tensor, src_lens, node_value_tensor, node_type_tensor, \
               node_action_tensor, node_lens, gnn_other_inputs
