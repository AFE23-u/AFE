...
class MixVocab(BaseVocab):
    def __init__(self, cmt: VocabEntry, code: VocabEntry, node: VocabEntry, action: VocabEntry, node_action: VocabEntry,
                 type: VocabEntry):
        self.cmt = cmt
        self.code = code
        self.node = node
    ...
    def load(file_path):
        with open(file_path, 'r') as f:
            entry = json.load(f)
        cmt_word2id = entry['cmt_vocab']
        code_word2id = entry['code_vocab']
        node_word2id = entry['node_vocab']
        action_word2id = entry['action_vocab']
        node_action_word2id = entry['node_action_vocab']
        type_word2id = entry['node_type_vocab']

        return MixVocab(VocabEntry(cmt_word2id), VocabEntry(code_word2id), VocabEntry(node_word2id),
                        VocabEntry(action_word2id), VocabEntry(node_action_word2id), VocabEntry(type_word2id))
    ...
    def load_embeddings(self, file_path: str):
        ...
        self.action.embeddings = embedding_dict['action_embeddings']
        self.node_type.embeddings = embedding_dict['type_embeddings']

    def __repr__(self):
        return 'MixVocab(cmt %d words, code %d words, node %d words, node_type %d words, action %d words, node_action %d words)' % (
            len(self.cmt), len(self.code), len(self.node), len(self.node_type), len(self.action), len(self.node_action))

    def get_subvocabs(self) -> Tuple[VocabEntry, VocabEntry, VocabEntry]:
        """
        :return: code vocab, nl vocab and action vocab
        """
        return self.code, self.cmt, self.node, self.node_type, self.action, self.node_action
