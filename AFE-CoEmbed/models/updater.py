...
class BaseUpdater(base.BaseModel, ABC):
    ...
    def __init__(self, vocab: Union[Vocab, MixVocab],
                 ...
                 gnn_layer_timesteps: int = GNN_LAYER_TIMESTEPS,
        ...
        self.mix_vocab = False
        # todo
        self.use_pre_embed = False
        self.freeze_pre_embed = False
        ...
        self.code_vocab, self.nl_vocab, self.node_vocab, self.node_type_vocab, self.action_vocab, self.node_action_vocab = self.vocab.get_subvocabs()
    ...
    def prepare_decoder_params(self):
        return self.nl_vocab, self.encoder.nl_embed_layer, self.encoder.edit_output_size, self.encoder.node_output_size, \
    ...
    def prepare_decoder_kwargs(self, encoder_output: dict, batch: Batch) -> dict:
        ...
        dec_init_state = self._prepare_dec_init_state([src_last_cell, edit_last_cell, node_last_cell])
    ...
    def forward(self, batch: Batch) -> Tensor:
        input_tensor = batch.construct_encoder_input(self.code_vocab, self.action_vocab, self.node_action_vocab,
                                                     self.nl_vocab, self.node_vocab,
                                                     self.node_type_vocab, self.device)
    ...
    def beam_search(self, example: Example, beam_size: int, max_dec_step: int,
        ...
        batch = Batch([example])
        input_tensor = batch.construct_encoder_input(self.code_vocab, self.action_vocab,self.node_action_vocab, self.nl_vocab, self.node_vocab,
                                                     self.node_type_vocab, self.device)
        encoder_output = self.encoder(*input_tensor)
        decoder_kwargs = self.prepare_decoder_kwargs(encoder_output, batch)
...
class CoAttnBPBAUpdater(BaseBPBAUpdater):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.encoder = CoAttnEncoder(self.code_vocab, self.action_vocab, self.nl_vocab, self.node_type_vocab,
                                     self.node_vocab, self.node_action_vocab,
                                     self.embed_size, self.edit_vec_size, self.enc_hidden_size, self.gnn_hidden_size,
                                     self.gnn_layer_timesteps, self.dropout,
                                     self.mix_vocab)
