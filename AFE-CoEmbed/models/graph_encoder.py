...
import config
from models.gnn import GatedGraphNeuralNetwork, AdjacencyList


class GNNEncoder(nn.Module):
    def __init__(self, node_embed_size, gnn_hidden_size, num_edge_types, dropout, gnn_layer_timesteps, device):
        super(GNNEncoder, self).__init__()
        self.gnn_hidden_size = gnn_hidden_size
        self.num_edge_types = num_edge_types
        self.embed_output_layer = nn.Linear(node_embed_size, self.gnn_hidden_size, bias=False)
    ...
    def forward(self, node_embedding, node_lens, node_as_output, edge_prt2ch,
        ...
        node_embedding = self.embed_output_layer(node_embedding)

        batch_node_vec = []
        batch_graph_vec = []
        batch_size = node_embedding.shape[1]
