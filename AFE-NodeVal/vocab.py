...
class MixVocab(BaseVocab):
    def __init__(self, token: VocabEntry, action: VocabEntry, node_type: VocabEntry):
        self.token = token
        self.action = action
    ...
    def load(file_path):
        with open(file_path, 'r') as f:
            entry = json.load(f)
        token_word2id = entry['mix_vocab']
        action_word2id = entry['action_vocab']
        type_word2id = entry['node_type_vocab']

        return MixVocab(VocabEntry(token_word2id), VocabEntry(action_word2id), VocabEntry(type_word2id))
    ...
    def __repr__(self):
        return 'MixVocab(token %d words, action %d words, node_type %d words)' % (
        len(self.token), len(self.action), len(self.node_type))

    def get_subvocabs(self) -> Tuple[VocabEntry, VocabEntry, VocabEntry]:
        """
        :return: code vocab, nl vocab and action vocab
        """
        return self.token, self.token, self.token, self.node_type, self.action
