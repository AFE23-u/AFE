...
class BaseEditor(AbstractEditor, ABC):
    def __init__(self, dec_vocab: VocabEntry, enc_embed_layer: nn.Module, edit_out_size: int, node_out_size: int,
                 src_out_size: int, embed_size: int, dec_hidden_size: int, teacher_forcing: float, share_embed: bool,
                 dropout: float):
        super().__init__()
...
class BothAttnSeqEditor(BaseEditor):
    def __init__(self, dec_vocab: VocabEntry, enc_embed_layer: nn.Module, edit_out_size: int, node_out_size: int,
        ...
        super(BothAttnSeqEditor, self).__init__(dec_vocab, enc_embed_layer, edit_out_size, node_out_size, src_out_size,
                                                embed_size, dec_hidden_size, teacher_forcing, share_embed, dropout)

        self.input_feed = input_feed
        self.attn_func = attn_func

        self.attention = BothAttention(self.attn_func, self.src_out_size, self.edit_out_size, self.node_out_size,
                                       self.hidden_size)
        ...
        self.att_node_linear = nn.Linear(self.node_out_size, self.hidden_size, bias=False)
    ...
    def _init_loop(self, **forward_args):
        ...
        node_encodings_att_linear = self.att_node_linear(forward_args['node_encodings'])
        static_input = (forward_args['src_encodings'], src_encodings_att_linear, forward_args['edit_encodings'],
                        edit_encodings_att_linear, forward_args['node_encodings'], node_encodings_att_linear,
                        forward_args['src_sent_masks'], forward_args['edit_sent_masks'],
                        forward_args['node_sent_masks'])
        return static_input
    ...
    def step(self, y_tm1_embed: Tensor, static_input: Any, state_tm1: Tuple) -> Tuple[Tuple, Tuple]:
        src_encodings, src_encodings_att_linear, edit_encodings, edit_encodings_att_linear, node_encodings, node_encodings_att_linear, \
        src_sent_masks, edit_sent_masks, node_sent_masks = static_input[:9]
        ...
        att_t, src_alpha_t, edit_alpha_t, node_alpha_t = self.attention(
            h_t, src_encodings, src_encodings_att_linear, edit_encodings, edit_encodings_att_linear, node_encodings,
            node_encodings_att_linear,
            src_sent_masks, edit_sent_masks, node_sent_masks)

        state_tm1 = ((h_t, cell_t), att_t)
        out_vec = att_t
        return state_tm1, (out_vec, src_alpha_t, edit_alpha_t, node_alpha_t)
...
class SrcPtrBASeqEditor(BothAttnSeqEditor):
    def __init__(self, dec_vocab: VocabEntry, enc_embed_layer: nn.Module, edit_out_size: int, node_out_size: int,
                 ...
                 embed_size: int, dec_hidden_size: int, teacher_forcing: float, share_embed: bool, dropout: float,
                 *args, **kwargs):
        super(SrcPtrBASeqEditor, self).__init__(dec_vocab, enc_embed_layer, edit_out_size, node_out_size, src_out_size,
                                                embed_size,
...
class BothPtrBASeqEditor(SrcPtrBASeqEditor):
    def __init__(self, dec_vocab: VocabEntry, enc_embed_layer: nn.Module, edit_out_size: int, node_out_size: int,
                 ...
                 embed_size: int, dec_hidden_size: int, teacher_forcing: float, share_embed: bool, dropout: float,
                 *args, **kwargs):
        super(BothPtrBASeqEditor, self).__init__(dec_vocab, enc_embed_layer, edit_out_size, node_out_size, src_out_size,
                                                 embed_size,
                                                 dec_hidden_size, teacher_forcing, share_embed, dropout, *args,
                                                 **kwargs)
        self.p_copy_src_linear = nn.Linear(self.hidden_size, 1)
        self.p_copy_code_linear = nn.Linear(self.hidden_size, 1)
    ...
    def prepare_prob_input(self, out_vecs: Union[Tuple[Tensor], List[Tuple[Tensor]]], **forward_kwargs) -> Tuple:
        ...
        static_input = [forward_kwargs['src_ext_tensor'], forward_kwargs['code_ext_tensor'],
                        forward_kwargs['node_ext_tensor'], forward_kwargs['max_ext_size']]
        if isinstance(out_vecs, Tuple):
            return tuple(list(out_vecs[:4]) + static_input)
        elif isinstance(out_vecs, List):
            # att_vec
            att_vec = torch.stack([out_v[0] for out_v in out_vecs], dim=0)
            # src_alpha_t
            ...
            # node_alpha_t
            node_att_weights = torch.stack([out_v[3] for out_v in out_vecs], dim=0)
            return tuple([att_vec, src_att_weights, code_att_weights, node_att_weights] + static_input)
        else:
            raise Exception("Unexpected type of out_vecs: {}".format(type(out_vecs)))
    ...
    def cal_words_log_prob(self, att_ves: Tensor, *args) -> DoubleTensor:
        ...
        :return: words_log_prob
        """
        src_att_weights, code_att_weights, node_att_weights, src_ext_tensor, code_ext_tensor, node_ext_tensor, max_ext_size = args[
                                                                                                                              0:7]
        src_ext_tensor = src_ext_tensor.transpose(1, 0)
        code_ext_tensor = code_ext_tensor.transpose(1, 0)
        node_ext_tensor = node_ext_tensor.transpose(1, 0)
        ...
        p_copy_code = torch.sigmoid(self.p_copy_code_linear(att_ves))

        src_copy_words_prob = (1 - p_gen) * p_copy_src * src_att_weights
        code_copy_words_prob = (1 - p_gen) * (1 - p_copy_src) * (1 - p_copy_code) * code_att_weights
        node_copy_words_prob = (1 - p_gen) * (1 - p_copy_src) * p_copy_code * node_att_weights
        ...
        # node copy
        node_copy_words_prob = node_copy_words_prob.double()
        words_prob = words_prob.scatter_add_(-1, node_ext_tensor.expand_as(node_copy_words_prob), node_copy_words_prob)
    ...
    def prepare_forward_kwargs(self, encoder_output: Tuple, batch: Batch, dec_init_state: Any, **kwargs):
        edit_encodings, edit_last_state, edit_last_cell, edit_sent_masks, src_encodings, src_last_state, \
        src_last_cell, src_sent_masks, node_encodings, node_last_state, node_last_cell, node_sent_masks = encoder_output
        nl_vocab = kwargs['nl_vocab']
        src_ext_tensor, code_ext_tensor, node_ext_tensor = batch.get_both_ext_tensor(nl_vocab, self.device)
        return {
            'src_encodings': src_encodings,
            'edit_encodings': edit_encodings,
            ...
            'src_sent_masks': src_sent_masks,
            'edit_sent_masks': edit_sent_masks,
            'node_sent_masks': node_sent_masks,
            'dec_init_state': dec_init_state,
            'src_ext_tensor': src_ext_tensor,
            'code_ext_tensor': code_ext_tensor,
            'node_ext_tensor': node_ext_tensor,
