...
def compute_metrics(src_file, ref_file, pred_file):
    ...
    with open('../prediction/result.json') as js:
        data = json.load(js)
    ...
    for ps in data:
            pres = [recover_desc(x[0]) for x in ps]
...
if __name__ == '__main__':
    src_file = "../prediction/src.txt"
    ref_file = '../prediction/ref.txt'
    pred_file = '../prediction/pred.txt'
